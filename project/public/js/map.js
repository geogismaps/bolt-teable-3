...
function detectURLMediaType(url, layerName = null, fieldName = null) {
    ...
    // Audio detection
    if (urlLower.includes('audio') || urlLower.includes('.mp3') || urlLower.includes('.wav') || urlLower.includes('.ogg') ||
...
window.zoomToAllLayers = function() {
    ...
    visibleLayers.forEach(layer => {
        layer.features.forEach(feature => {
            if ((feature.getLatLng && feature.getLatLng()) || (feature.getBounds && feature.getBounds().isValid())) {
...
function startMeasurement(type) {
    // Clear previous measurements if not in continuous mode
    if (!continuousMeasurement) {
        clearMeasurements();
    }

    // Update button states
    updateMeasurementButtonStates(type);
    ...
    if (type === 'distance') {
        ...
        showInfo('Click on map to start measuring distance. Double-click to finish.');
    } else if (type === 'area') {
        map.on('click', onAreaMeasureClick);
        map.on('dblclick', finishAreaMeasurement);
        showInfo('Click on map to start measuring area. Double-click to finish.');
    }
}

function updateMeasurementButtonStates(activeType) {
    const distanceBtn = document.getElementById('distanceMeasureBtn');
    const areaBtn = document.getElementById('areaMeasureBtn');

    // Reset button states
    distanceBtn.classList.remove('btn-primary');
    distanceBtn.classList.add('btn-outline-primary');
    areaBtn.classList.remove('btn-success');
    areaBtn.classList.add('btn-outline-success');

    // Highlight active button
    if (activeType === 'distance') {
        distanceBtn.classList.remove('btn-outline-primary');
        distanceBtn.classList.add('btn-primary');
    } else if (activeType === 'area') {
        areaBtn.classList.remove('btn-outline-success');
        areaBtn.classList.add('btn-success');
    }
}
...
function toggleMeasurementMode() {
    ...
    if (continuousMeasurement) {
        btn.innerHTML = '<i class="fas fa-edit me-1"></i>Continuous Mode: ON';
        btn.classList.remove('btn-outline-warning');
        btn.classList.add('btn-warning');
        showSuccess('Continuous measurement mode enabled. Measurements will accumulate.');
    } else {
        btn.innerHTML = '<i class="fas fa-edit me-1"></i>Continuous Mode: OFF';
        btn.classList.remove('btn-warning');
        btn.classList.add('btn-outline-warning');
        showInfo('Continuous measurement mode disabled.');
    }
}
...
function onDistanceMeasureClick(e) {
    ...
    const markerNumber = measurementPoints.length;
    ...
    L.marker(e.latlng, {
        icon: L.divIcon({
            className: 'measurement-marker-label',
            html: `<div style="background: #e74c3c; color: white; border-radius: 50%; width: 20px; height: 20px; text-align: center; line-height: 20px; font-size: 12px; font-weight: bold; border: 2px solid white;">${markerNumber}</div>`,
            iconSize: [20, 20],
            iconAnchor: [10, 10]
        })
    }).addTo(measurementGroup);
    ...
    if (measurementPoints.length > 1) {
        // Draw line segment
        const currentSegment = [measurementPoints[measurementPoints.length - 2], e.latlng];
        const line = L.polyline(currentSegment, {
            color: '#e74c3c',
            weight: 3,
            opacity: 0.8
        }).addTo(measurementGroup);
        ...
        const segmentDistance = calculateDistanceInUnits(currentSegment, getDistanceUnit());
        const totalDistance = calculateDistanceInUnits(measurementPoints, getDistanceUnit());
        ...
        const segmentLabel = formatDistance(segmentDistance, getDistanceUnit(), getDistancePrecision());
        const totalLabel = formatDistance(totalDistance, getDistanceUnit(), getDistancePrecision());

        L.marker(midpoint, {
            icon: L.divIcon({
                className: 'distance-segment-label',
                html: `<div style="background: rgba(231, 76, 60, 0.9); color: white; padding: 3px 6px; border-radius: 4px; font-size: 11px; border: 1px solid white; text-align: center;">
                        <div style="font-weight: bold;">${segmentLabel}</div>
                        <div style="font-size: 10px; opacity: 0.9;">Segment ${measurementPoints.length - 1}</div>
                       </div>`,
                iconSize: [80, 35],
                iconAnchor: [40, 17]
            })
        ...
        updateMeasurementResults('distance', totalDistance, totalLabel);
    }
}
...
function finishDistanceMeasurement(e) {
    if (measurementPoints.length > 1) {
        ...
        // Add final measurement to results
        const measurement = {
        ...
        // Add final total label at the end point
        L.marker(measurementPoints[measurementPoints.length - 1], {
            icon: L.divIcon({
                className: 'distance-total-label',
                html: `<div style="background: #27ae60; color: white; padding: 4px 8px; border-radius: 6px; font-size: 12px; font-weight: bold; border: 2px solid white; text-align: center;">
                        <div>Total Distance</div>
                        <div style="font-size: 14px;">${totalLabel}</div>
                        <div style="font-size: 10px;">Measurement #${measurementCounter}</div>
                       </div>`,
                iconSize: [120, 45],
                iconAnchor: [60, 22]
            })
        ...
        showSuccess(`Distance measurement completed: ${totalLabel}`);
        updateMeasurementResultsDisplay();
    }
    ...
    if (!continuousMeasurement) {
        updateMeasurementButtonStates(null);
...
function onAreaMeasureClick(e) {
    ...
    L.marker(e.latlng, {
        icon: L.divIcon({
            className: 'measurement-marker-label',
            html: `<div style="background: #27ae60; color: white; border-radius: 50%; width: 20px; height: 20px; text-align: center; line-height: 20px; font-size: 12px; font-weight: bold; border: 2px solid white;">${markerNumber}</div>`,
            iconSize: [20, 20],
            iconAnchor: [10, 10]
        })
    }).addTo(measurementGroup);
    ...
    if (measurementPoints.length > 2) {
        // Remove previous polygon to update
        ...
        // Calculate area
        const area = calculateAreaInUnits(measurementPoints, getAreaUnit());
        const areaLabel = formatArea(area, getAreaUnit(), getAreaPrecision());

        // Calculate perimeter
        const perimeter = calculatePerimeterInUnits(measurementPoints, getDistanceUnit());
        const perimeterLabel = formatDistance(perimeter, getDistanceUnit(), getDistancePrecision());

        // Add area label at centroid
        const centroid = polygon.getBounds().getCenter();
        L.marker(centroid, {
            icon: L.divIcon({
                className: 'area-temp-label',
                html: `<div style="background: rgba(39, 174, 96, 0.9); color: white; padding: 4px 8px; border-radius: 4px; font-size: 11px; border: 1px solid white; text-align: center;">
                        <div style="font-weight: bold;">${areaLabel}</div>
                        <div style="font-size: 10px;">Perimeter: ${perimeterLabel}</div>
                        <div style="font-size: 9px;">${measurementPoints.length} points</div>
                       </div>`,
                iconSize: [120, 40],
                iconAnchor: [60, 20]
            })
        ...
        updateMeasurementResults('area', area, areaLabel, perimeter, perimeterLabel);
    }
}
...
function finishAreaMeasurement(e) {
    if (measurementPoints.length > 2) {
        ...
        measurementGroup.eachLayer(layer => {
            if (layer.options && layer.options.className && layer.options.className.includes('area-temp-label')) {
                measurementGroup.removeLayer(layer);
            }
        });

        // Create final polygon
        const finalPolygon = L.polygon(measurementPoints, {
            color: '#27ae60',
            weight: 3,
            fillColor: '#27ae60',
            fillOpacity: 0.3
        ...
        // Add final label at centroid
        const centroid = finalPolygon.getBounds().getCenter();
        L.marker(centroid, {
            icon: L.divIcon({
                className: 'area-final-label',
                html: `<div style="background: #27ae60; color: white; padding: 5px 10px; border-radius: 6px; font-size: 12px; font-weight: bold; border: 2px solid white; text-align: center;">
                        <div>Area: ${areaLabel}</div>
                        <div style="font-size: 10px;">Perimeter: ${perimeterLabel}</div>
                        <div style="font-size: 9px;">Measurement #${measurementCounter}</div>
                       </div>`,
                iconSize: [140, 50],
                iconAnchor: [70, 25]
            })
        ...
        showSuccess(`Area measurement completed: ${areaLabel} (Perimeter: ${perimeterLabel})`);
        updateMeasurementResultsDisplay();
    }
...
// Enhanced calculation functions with proper unit conversions
function calculateDistanceInUnits(points, unit) {
    let totalDistance = 0;
    ...
    return totalDistance * DISTANCE_CONVERSIONS[unit];
}

function calculateAreaInUnits(points, unit) {
    if (points.length < 3) return 0;

    // Calculate area using more accurate spherical earth model
    let area = 0;
    ...
    // Calculate area using spherical excess method
    let excess = -(closedPoints.length - 2) * Math.PI;
    ...
    for (let i = 0; i < closedPoints.length - 1; i++) {
        ...
        const E = 2 * Math.atan2(Math.tan(deltaLng / 2) * (Math.tan(lat1 / 2) + Math.tan(lat2 / 2)), 
                                 2 + Math.tan(lat1 / 2) * Math.tan(lat2 / 2));
        excess += E;
    }
    ...
    return area * AREA_CONVERSIONS[unit];
}

function calculatePerimeterInUnits(points, unit) {
    ...
    // Add distance from last point to first to close the polygon
    if (points.length > 2) {
        perimeter += points[points.length - 1].distanceTo(points[0]);
    }

    // Convert from meters to requested unit
    return perimeter * DISTANCE_CONVERSIONS[unit];
}

function formatDistance(distance, unit, precision) {
    const unitLabels = {
        meters: 'm',
        kilometers: 'km',
        feet: 'ft',
    ...
    return `${distance.toFixed(precision)} ${unitLabels[unit]}`;
}

function formatArea(area, unit, precision) {
    const unitLabels = {
        square_meters: 'm²',
    ...
    return `${area.toFixed(precision)} ${unitLabels[unit]}`;
}

// Helper functions to get current unit settings
function getDistanceUnit() {
    return document.getElementById('distanceUnit').value;
}

function getAreaUnit() {
    return document.getElementById('areaUnit').value;
}

function getDistancePrecision() {
    return parseInt(document.getElementById('distancePrecision').value);
}

function getAreaPrecision() {
    return parseInt(document.getElementById('areaPrecision').value);
}

// Update measurement results display
function updateMeasurementResults(type, value, label, perimeter = null, perimeterLabel = null) {
    ...
    if (type === 'distance') {
        resultsList.innerHTML = `
            <div class="measurement-result">
                <strong>Current Distance:</strong> ${label}<br>
        ...
        resultsList.innerHTML = `
            <div class="measurement-result">
                <strong>Current Area:</strong> ${label}<br>
                <strong>Current Perimeter:</strong> ${perimeterLabel}<br>
                <small>Points: ${measurementPoints.length}</small>
            </div>
        `;
    }
...
function updateMeasurementResultsDisplay() {
    const resultsContainer = document.getElementById('measurementResults');
    const resultsList = document.getElementById('measurementResultsList');
    ...
    let html = '<div class="measurement-results-summary">';
    html += `<strong>Total Measurements: ${allMeasurements.length}</strong><br><br>`;

    allMeasurements.forEach((measurement, index) => {
        if (measurement.type === 'distance') {
            html += `
                <div class="measurement-item mb-2 p-2 border rounded">
                    <strong>Distance #${measurement.id}:</strong> ${measurement.label}<br>
                    <small class="text-muted">Points: ${measurement.points.length}</small>
                    <button class="btn btn-xs btn-outline-danger float-end" onclick="removeMeasurement(${index})">
                        <i class="fas fa-trash"></i>
                    </button>
                </div>
            `;
            ...
            html += `
                <div class="measurement-item mb-2 p-2 border rounded">
                    <strong>Area #${measurement.id}:</strong> ${measurement.label}<br>
                    <strong>Perimeter:</strong> ${measurement.perimeterLabel}<br>
                    <small class="text-muted">Points: ${measurement.points.length}</small>
                    <button class="btn btn-xs btn-outline-danger float-end" onclick="removeMeasurement(${index})">
                        <i class="fas fa-trash"></i>
                    </button>
...
function removeMeasurement(index) {
    if (index >= 0 && index < allMeasurements.length) {
        allMeasurements.splice(index, 1);
        updateMeasurementResultsDisplay();
        showInfo('Measurement removed from results list.');
    }
}
...
function clearMeasurements() {
    ...
    const modeBtn = document.getElementById('measurementModeBtn');
    if (modeBtn) {
        modeBtn.innerHTML = '<i class="fas fa-edit me-1"></i>Continuous Mode: OFF';
        modeBtn.classList.remove('btn-warning');
        modeBtn.classList.add('btn-outline-warning');
    }

    showSuccess('All measurements cleared.');
}
